...
class TestFastTransfers(unittest.TestCase):
    ...
    def test_fast_transfer_o(self):
        ...
        self.assertEqual(konto.saldo, 400 - 100 - 1, "Niepoprawne księgowanie!")
    ...
    def test_fast_transfer_to_company(self):
        konto = KontoFirmowe(self.nazwa, self.NIP)
        konto.saldo = 400
        konto.transfer_to_fast(400)
        self.assertEqual(konto.saldo, 400 - 400 - 5, "Niepoprawne księgowanie!")
    
    def test_if_i_have_enough_money_to_fasttransfer_company(self):
        konto = KontoFirmowe(self.nazwa, self.NIP)
        konto.saldo = 200
        konto.transfer_to_fast(201)
        self.assertEqual(konto.saldo, 200, "Konto wykonuje przelew pomimo za małego balansu!")
